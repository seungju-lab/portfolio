...
세 프로젝트 상세의 네 섹션·시스템 도식·링크·탐색을 구현했다.
[상세 웹 검증 기록](docs/verification/project-detail-web.md)에서 정적 빌드와 브라우저·기존 A4 인쇄의 검증 범위를 확인할 수 있다. 디자인 시안과 실제 웹 동작의 근거는 구별한다.

## 제공 PDF

[PDF 제공과 갱신](docs/pdf-serving.md)에 생성·검토 명령과 최신성 검사, 고정 주소의 캐시 정책을 기록했다. 소스 변경 후 `pnpm pdf:generate`로 파일을 갱신하고 검토한다. `pnpm build`는 오래되거나 누락된 PDF를 차단한다.
